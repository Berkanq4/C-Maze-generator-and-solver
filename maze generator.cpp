...
/*
void printMazeSolutionToFile(Maze& maze, const std::string& filename, vector <Cell*> path) { //this time for visualisation purposes.
    std::ofstream file(filename, std::ios::in | std::ios::out);  // Open file for reading and writing
    ...
    for (int i = 0; i < maze.M; ++i) {
        for (int j = 0; j < maze.N; ++j) {
            Cell* currentCell = &maze.cells[i * maze.N + j];
            //isPathAdjuster(path, currentCell);
